...
def main():
    ...
    parser.add_argument('--output-file', type=str, default=None, help='Optional path to save the output CSV file. Overrides default naming and date-based generation.')
    ...
    # --- Save to CSV ---
    output_dir_default = os.path.join(os.path.dirname(__file__), '..', '..', '..', '..', 'data', 'processed', 'villamichelin', 'Thermia')
    
    if args.output_file:
        output_file_path = args.output_file
        # Ensure the directory for the output file exists
        output_file_dir = os.path.dirname(output_file_path)
        if output_file_dir: # Check if output_file_dir is not an empty string (e.g. for relative paths in current dir)
            os.makedirs(output_file_dir, exist_ok=True)
    else:
        os.makedirs(output_dir_default, exist_ok=True) # Ensure default dir exists
        # Generate filename based on actual data range if possible
        if not resampled_df.empty and 'timestamp' in resampled_df.columns and not resampled_df['timestamp'].isna().all():
            min_date_str = resampled_df['timestamp'].min().strftime('%Y%m%d')
            max_date_str = resampled_df['timestamp'].max().strftime('%Y%m%d')
        else:
            # Fallback to requested date range if df is empty or no valid timestamps
            # This uses the 'end_date' (datetime.now()) and 'args.days' from the start of main()
            max_date_for_filename = end_date 
            min_date_for_filename = end_date - timedelta(days=args.days)
            min_date_str = min_date_for_filename.strftime('%Y%m%d')
            max_date_str = max_date_for_filename.strftime('%Y%m%d')
        
        output_file_path = os.path.join(output_dir_default, f"heat_pump_power_15min_{min_date_str}_to_{max_date_str}.csv")
    
    resampled_df.to_csv(output_file_path, index=False, date_format='%Y-%m-%d %H:%M:%S')
    
    print(f"\nSaved 15-min interval data with power calculations to {output_file_path}")
    if not resampled_df.empty:
        ...
        if args.plot:
            ...
            plot_power_input(output_file_path)
    else:
